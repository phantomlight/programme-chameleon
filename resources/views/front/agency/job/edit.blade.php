	...
	<div class="common-page-wrapper">
		<div class="container">
			<div class="row element-top-30 element-bottom-10">
				<div class="col-sm-8">
					<form class="sc-form clearfix" id="agencyEditJobForm" role="form" onsubmit="return false;" data-job="{{ $job->id }}">
						<div class="form-section">
							<div class="form-group">
								<label>Post Job on Behalf of:</label>
								<?php $companies = $agency->companies()->wherePivot('status', 'accept')->orderBy('name', 'desc')->get(); ?>
								@if (count($companies) > 0)
								<select name="company" class="form-control">
									@foreach ($companies as $company)
										<option value="{{ $_hash->encode($company->id) }}" @if ($job->company_id === $company->id) {{ 'selected="selected"' }} @endif>{{ $company->name }}</option>
									@endforeach
								</select>
								@else
									<div class="alert alert-danger">
										You can't post a job without any company's consent.
									</div>
								@endif
							</div>
							...
							<div class="form-group">
								...
								<span class="help-block">If you have add the duration, you have to use credit again. You currently have: {{ $agency->credit }} credit(s).></span>
							</div>
							@endif
